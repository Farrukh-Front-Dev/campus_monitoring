...
class APIClient:
    """Smart API client with retry logic, caching, and token management for CLI"""
    ...
    def authenticate(self):
        ...
        for attempt in range(CLI_CONFIG['max_retries']):
            try:
                data = {
                ...
                resp = self.session.post(
                    CLI_CONFIG['auth_url'], data=data, timeout=CLI_CONFIG['timeout']
                )
                ...
                if resp.status_code == 401:
                    ...
                    sys.exit(1)
                if attempt < CLI_CONFIG['max_retries'] - 1:
                    time.sleep(CLI_CONFIG['retry_delay'])
        ...
        for attempt in range(CLI_CONFIG['max_retries']):
            try:
                ...
                resp = self.session.get(
                    f"{CLI_CONFIG['base_url']}/{endpoint}",
                    headers=headers,
...
{Colors.RESET}""")
        print(f"    {Colors.GRAY}⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  "
              f"👤 {CLI_CONFIG['username']}{Colors.RESET}\n")
...
class Analytics:
    ...
    def __init__(self, client: APIClient):
        self.client = client
        self.login = CLI_CONFIG['username']
        self.collected_data = {}
    ...
    def cluster_map(self):
        ...
        data = self.client.get(f"clusters/{CLI_CONFIG['cluster_id']}/map?limit=250")
        if not data:
    ...
    def campuses(self):
        ...
        campuses_list = data.get('campuses', data if isinstance(data, list) else [])
...
class Exporter:
    ...
    def to_json(data, filename=None):
        """Export to JSON"""
        if not filename:
            filename = f"analytics_{CLI_CONFIG['username']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        try:
    ...
    def to_txt(data, filename=None):
        ...
        if not filename:
            filename = f"analytics_{CLI_CONFIG['username']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        try:
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(f"School21 Analytics Report\n")
                f.write(f"User: {CLI_CONFIG['username']}\n")
...
def run_all(analytics):
    """Barcha bo'limlarni ketma-ket ishga tushirish"""
    sections = [
